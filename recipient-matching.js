(function(root) {
  ...
  function address(value) {
    ...
    // Anchor the house number AFTER the street, never in a preceding QD/LT code.
    const rest = street[2];
    const parts = rest.match(/^(.+?),\s*(?:n(?:umero|ro)?\.?[\sº°]*)?(\d{1,5}[a-z]?)(?=\s|,|$)(.*)$/)
      || rest.match(/^([a-z][a-z\s.'-]*?)\s+(?:n(?:umero|ro)?\.?[\sº°]*)?(\d{1,5}[a-z]?)(?=\s|,|$)(.*)$/);
    if (!parts) return null;
    const kind = /^(r|rua)\.?$/.test(street[1]) ? 'rua'
      : /^(av|avenida)\.?$/.test(street[1]) ? 'avenida'
        : /^trav/.test(street[1]) ? 'travessa' : street[1];
  ...
  function extract(text) {
    ...
    for (let i = 0; i < focus.length; i++) {
      ...
      let addr = null;
      if (focus[i + 1]) addr = address(focus[i + 1].text);
      if (!addr && focus[i + 1] && focus[i + 2] && probableName(focus[i + 1].text) && address(focus[i + 2].text)) {
        name += ' ' + focus[++i].text;
        addr = address(focus[i + 1].text);
      }
      // Unit identifiers can be printed on their own line below the street.
      if (addr && focus[i + 2] && /^(apto|apartamento|ap|bloco|bl|sala|casa)\b/.test(normalize(focus[i + 2].text))) {
        addr = address(addr.text + ' ' + focus[i + 2].text);
      }
  ...
  function match(text, residents) {
    ...
    for (const resident of residents || []) {
      ...
      for (const block of blocks) {
        const read = nameTokens(block.name);
        ...
        const reasons = [exactName ? 'Nome completo coincide' : plausible ? 'Nome parcial ou com erro de leitura' : 'Nome diferente do cadastro',
          relation === 'exact' ? 'Rua e número coincidem' : relation === 'conflict' ? 'Endereço divergente' : 'Endereço ausente ou incompleto'];
        if (!best || score > best.score) best = { morador: resident, score, safe, exactName, plausible, relation, block, motivos: reasons };
      }
      if (!best && addresses.some(a => ['exact', 'incomplete'].includes(addressRelation(a, home)))) {
        best = { morador: resident, score: 15, safe: false, relation: 'address-only', motivos: ['Apenas endereço: confirme o destinatário'] };
      }
    ...
    // A similar name at the same home is ambiguous even when one spelling is exact.
    const competing = safe.length === 1 && candidates.some(c => c !== safe[0] && c.plausible && ['exact', 'incomplete'].includes(c.relation));
    const blockKeys = new Set(blocks.filter(b => b.address).map(b =>
      normalize(b.name) + '|' + b.address.street + '|' + b.address.number));
    ...
    else if (best && !best.plausible && chosenBlock) reason = 'O nome lido não corresponde ao morador cadastrado neste endereço. Não presuma vínculo entre eles.';
